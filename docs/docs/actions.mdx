...
## Module2Action Struct

The `Module2Action` struct defines an action that can be used both in HTTP routes and CLI commands. It is less tightly coupled with HTTP-specific definitions and can directly operate on sockets. Below is a detailed explanation of each field in the struct:
...
### `In`
- **Type:** `*Module2ActionBody`
- **Description:**  
  Defines the request body, similar to HTTP `body`. See `Module2ActionBody` for details.
...
- **Type:** `*Module2ActionBody`
- **Description:**  
  Defines the response body. See `Module2ActionBody` for details.

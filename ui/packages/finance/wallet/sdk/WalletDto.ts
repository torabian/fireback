...
/**
 * The base class definition for walletDto
 **/
export class WalletDto {
  ...
  applyFromObject(data = {}) {
    const d = data as Partial<WalletDto>;
  ...
  /**
   * Creates an instance of WalletDto, and possibleDtoObject
   * needs to satisfy the type requirement fully, otherwise typescript compile would
   * be complaining.
   **/
  static from(possibleDtoObject: WalletDtoType) {
    return new WalletDto(possibleDtoObject);
  }
  /**
   * Creates an instance of WalletDto, and partialDtoObject
   * needs to satisfy the type, but partially, and rest of the content would
   * be constructed according to data types and nullability.
   **/
  static with(partialDtoObject: PartialDeep<WalletDtoType>) {
    return new WalletDto(partialDtoObject);
  }
  copyWith(
    partial: PartialDeep<WalletDtoType>,
  ): InstanceType<typeof WalletDto> {
    return new WalletDto({ ...this.toJSON(), ...partial });
  }
  clone(): InstanceType<typeof WalletDto> {
    return new WalletDto(this.toJSON());
  }
}
export abstract class WalletDtoFactory {
  abstract create(data: unknown): WalletDto;
}
/**
 * The base type definition for walletDto
 **/
export type WalletDtoType = {
  /**
   *
   * @type {string}
   **/
...
export namespace WalletDtoType {}

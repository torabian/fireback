...
/**
 * Action to communicate with the action myWallets
 */
export type MyWalletsActionOptions = {
  queryKey?: unknown[];
  qs?: MyWalletsActionQueryParams;
};
export type MyWalletsActionQueryOptions = Omit<
  UseQueryOptions<unknown, unknown, GResponse<WalletViewDto>, unknown[]>,
  "queryKey"
> &
  MyWalletsActionOptions &
  Partial<{
...
export const useMyWalletsActionQuery = (
  options: MyWalletsActionQueryOptions,
) => {
  ...
  const result = useQuery({
    queryKey: [MyWalletsAction.NewUrl(options?.qs)],
    queryFn: fn,
    ...(options || {}),
  });
  return {
...
export type MyWalletsActionMutationOptions = Omit<
...
export const useMyWalletsAction = (
  options?: MyWalletsActionMutationOptions,
) => {
  const globalCtx = useFetchxContext();
  const ctx = options?.ctx ?? globalCtx ?? undefined;
  const [isCompleted, setCompleteState] = useState(false);
 ...
 * MyWalletsAction
 */
export class MyWalletsAction {
  //
  static URL = "/wallet/my";
  static NewUrl = (qs?: MyWalletsActionQueryParams) =>
    buildUrl(MyWalletsAction.URL, undefined, qs);
  static Method = "GET";
  static Fetch$ = async (
    qs?: MyWalletsActionQueryParams,
    ctx?: FetchxContext | null,
    ...
    return fetchx<GResponse<WalletViewDto>, unknown, unknown>(
      overrideUrl ?? MyWalletsAction.NewUrl(qs),
      {
        method: MyWalletsAction.Method,
        ...(init || {}),
      },
      ctx,
    );
  ...
  static Fetch = async (
    ...
    const res = await MyWalletsAction.Fetch$(qs, ctx, init, overrideUrl);
...
/**
 * MyWalletsActionQueryParams class
 * Auto-generated from EmiAction
 */
export class MyWalletsActionQueryParams extends URLSearchParamsX {
  /**
   *
   * @returns { string | null }
   */

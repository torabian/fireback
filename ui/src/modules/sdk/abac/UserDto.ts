...
export class UserDto {
   ...
   * @type {string}
   **/
  set lastIpAddress(value: string) {
    this.#lastIpAddress = String(value);
  }
   ...
   * @type {UserDto.PrimaryAddress}
   **/
  #primaryAddress?:
    | InstanceType<typeof UserDto.PrimaryAddress>
    | null
    | undefined
    | null = undefined;
  /**
   * User primary address location. Can be useful for simple projects that a user is associated with a single address.
   * @returns {UserDto.PrimaryAddress}
   **/
  get primaryAddress() {
  ...
  set primaryAddress(
    ...
    if (value instanceof UserDto.PrimaryAddress) {
      this.#primaryAddress = value;
    } else {
      this.#primaryAddress = new UserDto.PrimaryAddress(value);
    }
  }
  setPrimaryAddress(
    value:
  ...
  static PrimaryAddress = class PrimaryAddress {
    ...
    /**
     * Creates an instance of UserDto.PrimaryAddress, and possibleDtoObject
     * needs to satisfy the type requirement fully, otherwise typescript compile would
     * be complaining.
     **/
    static from(possibleDtoObject: UserDtoType.PrimaryAddressType) {
      return new UserDto.PrimaryAddress(possibleDtoObject);
    }
    /**
     * Creates an instance of UserDto.PrimaryAddress, and partialDtoObject
     * needs to satisfy the type, but partially, and rest of the content would
     * be constructed according to data types and nullability.
     **/
    static with(partialDtoObject: PartialDeep<UserDtoType.PrimaryAddressType>) {
      return new UserDto.PrimaryAddress(partialDtoObject);
    }
    copyWith(
      partial: PartialDeep<UserDtoType.PrimaryAddressType>,
    ): InstanceType<typeof UserDto.PrimaryAddress> {
      return new UserDto.PrimaryAddress({ ...this.toJSON(), ...partial });
    }
    clone(): InstanceType<typeof UserDto.PrimaryAddress> {
      return new UserDto.PrimaryAddress(this.toJSON());
    }
  };
  constructor(data: unknown = undefined) {
    if (data === null || data === undefined) {
   ...
   **/
  applyFromObject(data = {}) {
    const d = data as Partial<UserDto>;
  ...
  /**
   * Creates an instance of UserDto, and possibleDtoObject
   * needs to satisfy the type requirement fully, otherwise typescript compile would
   * be complaining.
   **/
  static from(possibleDtoObject: UserDtoType) {
    return new UserDto(possibleDtoObject);
  }
  /**
   * Creates an instance of UserDto, and partialDtoObject
   * needs to satisfy the type, but partially, and rest of the content would
   * be constructed according to data types and nullability.
   **/
  static with(partialDtoObject: PartialDeep<UserDtoType>) {
    return new UserDto(partialDtoObject);
  }
  copyWith(partial: PartialDeep<UserDtoType>): InstanceType<typeof UserDto> {
    return new UserDto({ ...this.toJSON(), ...partial });
  }
  clone(): InstanceType<typeof UserDto> {
    return new UserDto(this.toJSON());
  }
}
export abstract class UserDtoFactory {
  abstract create(data: unknown): UserDto;
}
/**
 * The base type definition for userDto
 **/
export type UserDtoType = {
  ...
  /**
   * User primary address location. Can be useful for simple projects that a user is associated with a single address.
   * @type {UserDtoType.PrimaryAddressType}
   **/

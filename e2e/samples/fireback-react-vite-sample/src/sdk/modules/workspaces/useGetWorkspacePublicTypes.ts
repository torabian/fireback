...
import { QueryWorkspaceTypesPubliclyActionResDto } from "../workspaces/WorkspacesActionsDto";
export function useGetWorkspacePublicTypes({
  ...
  const query$ = useQuery<
    any,
    any,
    IResponseList<QueryWorkspaceTypesPubliclyActionResDto>,
    any
  >(
    [
  ...
  const items: Array<QueryWorkspaceTypesPubliclyActionResDto> =
    query$.data?.data?.items || [];
  return {
    query: query$,
    items,
    keyExtractor: (item: QueryWorkspaceTypesPubliclyActionResDto) =>
      item.uniqueId,
  };
}

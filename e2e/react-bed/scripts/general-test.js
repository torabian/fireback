...
const tests = [
  {
    file: "./src/sdk/modules/fireback/AcceptInviteDto.ts",
    toContain: [
      "inviteUniqueId",
  ...
  {
    file: "./src/sdk/modules/fireback/useDeleteCapability.ts",
    toContain: [
  ...
  {
    file: "./src/sdk/modules/fireback/useGetPassports.ts",
    toContain: [
      'const fn = () => rpcFn("GET", computedUrl);',
      'const query$ = useQuery<any, any, IResponseList<PassportEntity>, any>(["*fireback.PassportEntity", computedOptions, query], fn, {',
      'useGetPassports.UKEY = "*fireback.PassportEntity"',
...
function runTest() {
  for (let test of tests) {
    ...
    if (fs.existsSync(test.file) === false) {
      console.log(
        "Unfortunately during the test file:",
        test.file,
        " was not found."
      );
      process.exit(1);
    ...
    if (test.toContain) {
      ...
      if (Array.isArray(test.toContain)) {
        for (let t of test.toContain) {
          if (body.indexOf(t) === -1) {
            console.log(
              "Unfortunately we expected to find '",
              t,
              "' in the file:",
              test.file,
              "but was not found"

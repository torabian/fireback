...
*/
	...
	/**
 * FetchImportUserAction
 */

export class FetchImportUserAction {
  static URL = '/user/import';
  static Method = 'post';
	...
	static Fetch = (
		...
		qs?: ImportUserQueryParams,
		overrideUrl?: string
	) =>
		fetchx<unknown, unknown, unknown>(
			new URL((overrideUrl ?? FetchImportUserAction.URL ) + '?' + qs?.toString()),
			init
		)
...
setPath (value) { this[`path`] = value; return this; } 
   ...
   * @example
   * @Get()
   * getHello(@ImportUserReq.Nest() headers: ImportUserReq): string {
  ...
  static Nest = createParamDecorator(
	(_data, ctx) => {
		...
		// @ts-ignore
		return new ImportUserReq( request.body );
	},
  );
...
export class ImportUserHeaders extends Headers {
   ...
   * @example
   * @Get()
   * getHello(@ImportUserHeaders.Nest() headers: ImportUserHeaders): string {
   *  return JSON.stringify(headers.getContentType());
   * }
   */
  static Nest = createParamDecorator(
...
export class ImportUserQueryParams extends URLSearchParamsX {
   ...
   * @example
   * @Get()
   * getHello(@ImportUserQueryParams.Nest() query: ImportUserQueryParams): string {

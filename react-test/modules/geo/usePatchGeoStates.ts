...
export function usePatchGeoStates({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  query = query || {}
  ...
  const fnUpdater: any = (
    data: PossibleStoreData<GeoStateEntity>,
    response: IResponse<BulkRecordRequest<GeoStateEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<GeoStateEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[geo.GeoStateEntity]", (data: any) =>

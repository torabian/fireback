...
export function usePatchNotificationConfigs({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  ...
  const fnUpdater: any = (
    ...
    response: IResponse<BulkRecordRequest<NotificationConfigEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<NotificationConfigEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[workspaces.NotificationConfigEntity]", (data: any) =>

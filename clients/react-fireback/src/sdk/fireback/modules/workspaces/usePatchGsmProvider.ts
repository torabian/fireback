...
export function usePatchGsmProvider({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchGsmProvider(entity);

  const mutation = useMutation<
    IResponse<workspaces.GsmProviderEntity>,
    IResponse<workspaces.GsmProviderEntity>,
    Partial<workspaces.GsmProviderEntity>
  >(fn);
  ...
  const submit = (
    values: Partial<workspaces.GsmProviderEntity>,
    formikProps?: FormikHelpers<Partial<workspaces.GsmProviderEntity>>

...
export function usePostPassportSignupEmail({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().postPassportSignupEmail(entity);

  const mutation = useMutation<
    IResponse<workspaces.UserSessionDto>,
    IResponse<workspaces.UserSessionDto>,
    Partial<workspaces.EmailAccountSignupDto>
  >(fn);
  ...
  const submit = (
    values: Partial<workspaces.EmailAccountSignupDto>,

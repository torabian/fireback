...
export function usePatchGeoLocationType(props?: PatchProps) {
  ...
  const url = "/geo-location-type".substr(1);
  ...
  const fnUpdater = (
    data: IResponseList<GeoLocationTypeEntity> | undefined,
    item: IResponse<GeoLocationTypeEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<GeoLocationTypeEntity>) {
          queryClient?.setQueriesData("*geo.GeoLocationTypeEntity", (data: any) =>

...
export function useGetPageCategoriesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/page-categories/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<PageCategoryEntity>, any>(["*cms.PageCategoryEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<PageCategoryEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetPageCategoriesExport.UKEY = "*cms.PageCategoryEntity"
...
export function useGetProductFieldsByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/product/fields/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*shop.ProductFields"], fn, {
    cacheTime: 1001,
    retry: false,

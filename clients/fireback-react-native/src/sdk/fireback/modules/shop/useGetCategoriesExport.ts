...
export function useGetCategoriesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/categories/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<CategoryEntity>, any>(["*shop.CategoryEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<CategoryEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetCategoriesExport.UKEY = "*shop.CategoryEntity"
...
export function useGetDiscountCodesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/discount-codes/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<DiscountCodeEntity>, any>(["*shop.DiscountCodeEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<DiscountCodeEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetDiscountCodesExport.UKEY = "*shop.DiscountCodeEntity"
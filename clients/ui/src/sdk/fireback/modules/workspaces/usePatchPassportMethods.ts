...
export function usePatchPassportMethods({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  ...
  const fnUpdater: any = (
    ...
    response: IResponse<BulkRecordRequest<PassportMethodEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<PassportMethodEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[workspaces.PassportMethodEntity]", (data: any) =>

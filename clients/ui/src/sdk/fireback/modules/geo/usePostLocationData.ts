...
export function usePostLocationData(props?: UseRemoteQuery) {
  ...
  const url = "/location-data".substr(1);
  ...
  const fnUpdater = (
    data: IResponseList<LocationDataEntity> | undefined,
    item: IResponse<LocationDataEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<LocationDataEntity>) {
          queryClient?.setQueryData<IResponseList<LocationDataEntity>>(

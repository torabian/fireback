import Promises
func PostEmailSend(dto: SendEmailActionReqDto) -> Promise<SendEmailActionResDto?> {
    return Promise<SendEmailActionResDto?>(on: .main) { fulfill, reject in
  ...
  let url = URL(string: prefix + "//email/send")!
  var request = URLRequest(url: url)
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            if let data = data {
                ...
                do {
                    let result = try decoder.decode(SingleResponse<SendEmailActionResDto>.self, from: data)
                    if result.error != nil {
                        reject(result.error!)
                    } else {
                        fulfill(result.data)

...
import Combine
import SwiftUI
func GetCapabilitiesExportFetcher() -> AnyPublisher<ArrayResponse<CapabilityEntity>, Error> {
    var computedUrl = "http://localhost:61901/capabilities/export"
    ...
    var computedUrl = "http://localhost:61901/capabilities/export"
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            if let data = data {
                ...
                do {
                    let result = try decoder.decode(SingleResponse<CapabilityEntity>.self, from: data)
                    if result.error != nil {
                        reject(result.error!)
                    } else {
                        fulfill(result.data)

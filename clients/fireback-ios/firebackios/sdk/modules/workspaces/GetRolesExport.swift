...
import Combine
import SwiftUI
func GetRolesExportFetcher() -> AnyPublisher<ArrayResponse<RoleEntity>, Error> {
    var computedUrl = "http://localhost:61901/roles/export"
    ...
    return URLSession.shared
        ...
        .map(\.data)
        .decode(type: ArrayResponse<RoleEntity>.self, decoder: JSONDecoder())
        .receive(on: DispatchQueue.main)
        .eraseToAnyPublisher()
}
/*
I have commented this for now, because this is not returning correctly, as well as the function above is good enough
func GetRolesExport() -> Promise<[RoleEntity]?> {
    return Promise<[RoleEntity]?>(on: .main) { fulfill, reject in
    var computedUrl = "http://localhost:61901/roles/export"
        var request = URLRequest(url: URL(string: computedUrl)!)
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            if let data = data {
                ...
                do {
                    let result = try decoder.decode(SingleResponse<RoleEntity>.self, from: data)
                    if result.error != nil {
                        reject(result.error!)
                    } else {
                        fulfill(result.data)

...
import Combine
import SwiftUI
func GetHmisExportFetcher() -> AnyPublisher<ArrayResponse<HmiEntity>, Error> {
    var computedUrl = "http://localhost:61901/hmis/export"
    ...
    return URLSession.shared
        ...
        .map(\.data)
        .decode(type: ArrayResponse<HmiEntity>.self, decoder: JSONDecoder())
        .receive(on: DispatchQueue.main)
        .eraseToAnyPublisher()
}
/*
I have commented this for now, because this is not returning correctly, as well as the function above is good enough
func GetHmisExport() -> Promise<[HmiEntity]?> {
    return Promise<[HmiEntity]?>(on: .main) { fulfill, reject in
    var computedUrl = "http://localhost:61901/hmis/export"
        var request = URLRequest(url: URL(string: computedUrl)!)
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            if let data = data {
                ...
                do {
                    let result = try decoder.decode(SingleResponse<HmiEntity>.self, from: data)
                    if result.error != nil {
                        reject(result.error!)
                    } else {
                        fulfill(result.data)

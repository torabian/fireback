...
import Combine
import SwiftUI
func GetHmiComponentTypesExportFetcher() -> AnyPublisher<ArrayResponse<HmiComponentTypeEntity>, Error> {
    var computedUrl = "http://localhost:61901/hmi-component-types/export"
    ...
    return URLSession.shared
        ...
        .map(\.data)
        .decode(type: ArrayResponse<HmiComponentTypeEntity>.self, decoder: JSONDecoder())
        .receive(on: DispatchQueue.main)
        .eraseToAnyPublisher()
}
/*
I have commented this for now, because this is not returning correctly, as well as the function above is good enough
func GetHmiComponentTypesExport() -> Promise<[HmiComponentTypeEntity]?> {

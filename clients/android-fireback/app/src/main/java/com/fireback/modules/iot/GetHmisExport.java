...
/*
*/
public class GetHmisExport {
    private Context context;
    public GetHmisExport(Context ctx ) {
        context = ctx;
    }
    public static String Url  = FirebackConfig.getInstance().BuildUrl("/hmis/export");
    ...
    public Observable<ArrayResponse<HmiEntity>> query() {
        return Observable.just("")
            .observeOn(Schedulers.io())
            .map(tick -> makeHttpRequest())
            .map(response -> {
                if (response.isSuccessful()) {
                    ArrayResponse<HmiEntity> res = ArrayResponse.fromJson(response.body().string(), HmiEntity.class);
                    response.close();
                    return res;
                } else {

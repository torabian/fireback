...
/*
*/
public class GetUsersExport {
    private Context context;
    public GetUsersExport(Context ctx ) {
        context = ctx;
    }
    public static String Url  = FirebackConfig.getInstance().BuildUrl("/users/export");
        ...
        return Observable.just("")
            ...
            .map(response -> {
                if (response.isSuccessful()) {
                    ArrayResponse<UserEntity> res = ArrayResponse.fromJson(response.body().string(), UserEntity.class);
                    response.close();
                    return res;
                } else {

...
public class GetRoles {
    ...
    public GetRoles(Context ctx ) {
        context = ctx;
    }
    public static String Url  = FirebackConfig.getInstance().BuildUrl("/roles");
        ...
        return Observable.just("")
            ...
            .map(response -> {
                if (response.isSuccessful()) {
                    ArrayResponse<RoleEntity> res = ArrayResponse.fromJson(response.body().string(), RoleEntity.class);
                    response.close();
                    return res;
                } else {

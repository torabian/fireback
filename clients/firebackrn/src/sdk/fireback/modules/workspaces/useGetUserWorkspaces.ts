...
export function useGetUserWorkspaces({
  ...
  const query$ = useQuery<any, any, IResponseList<UserWorkspaceEntity>, any>(["*workspaces.UserWorkspaceEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<UserWorkspaceEntity> = query$.data?.data?.items || [];

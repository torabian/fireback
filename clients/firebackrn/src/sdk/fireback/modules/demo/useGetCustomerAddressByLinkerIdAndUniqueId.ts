...
export function useGetCustomerAddressByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/customer/address/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*demo.CustomerAddress"], fn, {
    cacheTime: 1001,
    retry: false,

...
export function useGetTagsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/tags/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<TagEntity>, any>(["*shop.TagEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<TagEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetTagsExport.UKEY = "*shop.TagEntity"
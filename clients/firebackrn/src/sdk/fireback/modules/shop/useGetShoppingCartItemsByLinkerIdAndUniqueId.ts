...
export function useGetShoppingCartItemsByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/shopping-cart/items/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*shop.ShoppingCartItems"], fn, {
    cacheTime: 1001,
    retry: false,

...
export function useGetOrderItemsByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/order/items/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*shop.OrderItems"], fn, {
    cacheTime: 1001,
    retry: false,

...
export function useGetOrderStatusesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/order-statuses/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<OrderStatusEntity>, any>(["*shop.OrderStatusEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<OrderStatusEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetOrderStatusesExport.UKEY = "*shop.OrderStatusEntity"
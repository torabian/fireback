...
export function useGetProductSubmissionsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/product-submissions/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<ProductSubmissionEntity>, any>(["*shop.ProductSubmissionEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<ProductSubmissionEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetProductSubmissionsExport.UKEY = "*shop.ProductSubmissionEntity"
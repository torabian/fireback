...
export function useGetProductSubmissionByUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/product-submission/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*shop.ProductSubmissionEntity"], fn, {
    cacheTime: 1001,
    retry: false,

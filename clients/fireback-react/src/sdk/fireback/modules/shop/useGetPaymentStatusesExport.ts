...
export function useGetPaymentStatusesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/payment-statuses/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<PaymentStatusEntity>, any>(["*shop.PaymentStatusEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<PaymentStatusEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetPaymentStatusesExport.UKEY = "*shop.PaymentStatusEntity"
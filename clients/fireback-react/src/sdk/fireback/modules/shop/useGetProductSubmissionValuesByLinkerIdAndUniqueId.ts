...
export function useGetProductSubmissionValuesByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/product-submission/values/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*shop.ProductSubmissionValues"], fn, {
    cacheTime: 1001,
    retry: false,

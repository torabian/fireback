...
export function useGetPaymentMethodsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/payment-methods/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<PaymentMethodEntity>, any>(["*shop.PaymentMethodEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<PaymentMethodEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetPaymentMethodsExport.UKEY = "*shop.PaymentMethodEntity"
...
export function useGetOrderTotalPriceByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/order/total_price/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*shop.OrderTotalPrice"], fn, {
    cacheTime: 1001,
    retry: false,

...
export function useGetProductSubmissionPriceByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/product-submission/price/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*shop.ProductSubmissionPrice"], fn, {
    cacheTime: 1001,
    retry: false,

...
export function useGetOrdersExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/orders/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<OrderEntity>, any>(["*shop.OrderEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<OrderEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetOrdersExport.UKEY = "*shop.OrderEntity"
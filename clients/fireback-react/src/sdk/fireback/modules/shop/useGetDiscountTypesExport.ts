...
export function useGetDiscountTypesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/discount-types/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<DiscountTypeEntity>, any>(["*shop.DiscountTypeEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<DiscountTypeEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetDiscountTypesExport.UKEY = "*shop.DiscountTypeEntity"
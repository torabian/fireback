...
export function useGetWorkspaceInvites({
  ...
  const query$ = useQuery<any, any, IResponseList<WorkspaceInviteEntity>, any>(["*workspaces.WorkspaceInviteEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<WorkspaceInviteEntity> = query$.data?.data?.items || [];

...
export function usePatchUserWorkspaces({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  ...
  const fnUpdater: any = (
    ...
    response: IResponse<BulkRecordRequest<UserWorkspaceEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<UserWorkspaceEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[workspaces.UserWorkspaceEntity]", (data: any) =>

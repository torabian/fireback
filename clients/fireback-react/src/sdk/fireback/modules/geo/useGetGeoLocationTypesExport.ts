...
export function useGetGeoLocationTypesExport({
  ...
  const url = "/geo-location-types/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<GeoLocationTypeEntity>, any>(["*geo.GeoLocationTypeEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<GeoLocationTypeEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetGeoLocationTypesExport.UKEY = "*geo.GeoLocationTypeEntity"
...
export function usePostGeoCity(props?: UseRemoteQuery) {
  ...
  const fnUpdater = (
    data: IResponseList<GeoCityEntity> | undefined,
    item: IResponse<GeoCityEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<GeoCityEntity>) {
          queryClient?.setQueryData<IResponseList<GeoCityEntity>>(
            "*geo.GeoCityEntity",

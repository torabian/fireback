...
/*
*/
public class GetFilesExport {
    private Context context;
    public GetFilesExport(Context ctx ) {
        context = ctx;
    }
    public static String Url  = FirebackConfig.getInstance().BuildUrl("/files/export");
    ...
    public Observable<ArrayResponse<FileEntity>> query() {
        return Observable.just("")
            .observeOn(Schedulers.io())
            .map(tick -> makeHttpRequest())
            .map(response -> {
                if (response.isSuccessful()) {
                    ArrayResponse<FileEntity> res = ArrayResponse.fromJson(response.body().string(), FileEntity.class);
                    response.close();
                    return res;
                } else {

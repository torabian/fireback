...
/*
import com.fireback.BrandEntity;
*/
...
public class PostBrand {
    public static String Url  = FirebackConfig.getInstance().BuildUrl("/brand");
    public Single<SingleResponse<BrandEntity>> post(BrandEntity dto) {
        return Single.fromCallable(() -> makeHttpPostRequest(dto))
                .subscribeOn(Schedulers.io());
    }
    private SingleResponse<BrandEntity> makeHttpPostRequest(BrandEntity dto) throws ResponseErrorException {
        ...
        try (Response response = client.newCall(request).execute()) {
            if (response.isSuccessful()) {
                SingleResponse<BrandEntity> res = SingleResponse.fromJson(response.body().string(), BrandEntity.class);
                response.close();
                return res;
            } else {

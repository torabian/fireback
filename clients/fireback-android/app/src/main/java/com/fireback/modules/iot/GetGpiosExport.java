...
/*
*/
public class GetGpiosExport {
    private Context context;
    public GetGpiosExport(Context ctx ) {
        context = ctx;
    }
    public static String Url  = FirebackConfig.getInstance().BuildUrl("/gpios/export");
    ...
    public Observable<ArrayResponse<GpioEntity>> query() {
        return Observable.just("")
            .observeOn(Schedulers.io())
            .map(tick -> makeHttpRequest())
            .map(response -> {
                if (response.isSuccessful()) {
                    ArrayResponse<GpioEntity> res = ArrayResponse.fromJson(response.body().string(), GpioEntity.class);
                    response.close();
                    return res;
                } else {

...
/*
*/
public class GetDevicesExport {
    private Context context;
    public GetDevicesExport(Context ctx ) {
        context = ctx;
    }
    public static String Url  = FirebackConfig.getInstance().BuildUrl("/devices/export");
    ...
    public Observable<ArrayResponse<DeviceEntity>> query() {
        return Observable.just("")
            .observeOn(Schedulers.io())
            .map(tick -> makeHttpRequest())
            .map(response -> {
                if (response.isSuccessful()) {
                    ArrayResponse<DeviceEntity> res = ArrayResponse.fromJson(response.body().string(), DeviceEntity.class);
                    response.close();
                    return res;
                } else {

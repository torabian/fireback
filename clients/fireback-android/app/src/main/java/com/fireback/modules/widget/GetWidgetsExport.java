...
/*
*/
public class GetWidgetsExport {
    private Context context;
    public GetWidgetsExport(Context ctx ) {
        context = ctx;
    }
    public static String Url  = FirebackConfig.getInstance().BuildUrl("/widgets/export");
    ...
    public Observable<ArrayResponse<WidgetEntity>> query() {
        return Observable.just("")
            .observeOn(Schedulers.io())
            .map(tick -> makeHttpRequest())
            .map(response -> {
                if (response.isSuccessful()) {
                    ArrayResponse<WidgetEntity> res = ArrayResponse.fromJson(response.body().string(), WidgetEntity.class);
                    response.close();
                    return res;
                } else {

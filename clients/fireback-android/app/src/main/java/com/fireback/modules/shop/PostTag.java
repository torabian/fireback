...
public class PostTag {
    public static String Url  = FirebackConfig.getInstance().BuildUrl("/tag");
    public Single<SingleResponse<TagEntity>> post(TagEntity dto) {
        return Single.fromCallable(() -> makeHttpPostRequest(dto))
                .subscribeOn(Schedulers.io());
    }
    private SingleResponse<TagEntity> makeHttpPostRequest(TagEntity dto) throws ResponseErrorException {
        ...
        try (Response response = client.newCall(request).execute()) {
            if (response.isSuccessful()) {
                SingleResponse<TagEntity> res = SingleResponse.fromJson(response.body().string(), TagEntity.class);
                response.close();
                return res;
            } else {

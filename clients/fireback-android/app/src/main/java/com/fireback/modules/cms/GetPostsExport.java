...
/*
*/
public class GetPostsExport {
    private Context context;
    public GetPostsExport(Context ctx ) {
        context = ctx;
    }
    public static String Url  = FirebackConfig.getInstance().BuildUrl("/posts/export");
    ...
    public Observable<ArrayResponse<PostEntity>> query() {
        return Observable.just("")
            .observeOn(Schedulers.io())
            .map(tick -> makeHttpRequest())
            .map(response -> {
                if (response.isSuccessful()) {
                    ArrayResponse<PostEntity> res = ArrayResponse.fromJson(response.body().string(), PostEntity.class);
                    response.close();
                    return res;
                } else {

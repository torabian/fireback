...
/*
import com.fireback.OrderEntity;
*/
...
public class PostOrder {
    public static String Url  = FirebackConfig.getInstance().BuildUrl("/order");
    public Single<SingleResponse<OrderEntity>> post(OrderEntity dto) {
        return Single.fromCallable(() -> makeHttpPostRequest(dto))
                .subscribeOn(Schedulers.io());
    }
    private SingleResponse<OrderEntity> makeHttpPostRequest(OrderEntity dto) throws ResponseErrorException {
        ...
        try (Response response = client.newCall(request).execute()) {
            if (response.isSuccessful()) {
                SingleResponse<OrderEntity> res = SingleResponse.fromJson(response.body().string(), OrderEntity.class);
                response.close();
                return res;
            } else {

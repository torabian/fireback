...
/*
import com.fireback.PostEntity;
*/
...
public class PostPost {
    public static String Url  = FirebackConfig.getInstance().BuildUrl("/post");
    public Single<SingleResponse<PostEntity>> post(PostEntity dto) {
        return Single.fromCallable(() -> makeHttpPostRequest(dto))
                .subscribeOn(Schedulers.io());
    }
    private SingleResponse<PostEntity> makeHttpPostRequest(PostEntity dto) throws ResponseErrorException {
        ...
        try (Response response = client.newCall(request).execute()) {
            if (response.isSuccessful()) {
                SingleResponse<PostEntity> res = SingleResponse.fromJson(response.body().string(), PostEntity.class);
                response.close();
                return res;
            } else {

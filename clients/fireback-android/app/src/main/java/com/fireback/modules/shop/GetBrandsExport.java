...
/*
*/
public class GetBrandsExport {
    private Context context;
    public GetBrandsExport(Context ctx ) {
        context = ctx;
    }
    public static String Url  = FirebackConfig.getInstance().BuildUrl("/brands/export");
    ...
    public Observable<ArrayResponse<BrandEntity>> query() {
        return Observable.just("")
            .observeOn(Schedulers.io())
            .map(tick -> makeHttpRequest())
            .map(response -> {
                if (response.isSuccessful()) {
                    ArrayResponse<BrandEntity> res = ArrayResponse.fromJson(response.body().string(), BrandEntity.class);
                    response.close();
                    return res;
                } else {

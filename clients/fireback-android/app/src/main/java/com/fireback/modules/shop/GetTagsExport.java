...
/*
*/
public class GetTagsExport {
    private Context context;
    public GetTagsExport(Context ctx ) {
        context = ctx;
    }
    public static String Url  = FirebackConfig.getInstance().BuildUrl("/tags/export");
    ...
    public Observable<ArrayResponse<TagEntity>> query() {
        return Observable.just("")
            .observeOn(Schedulers.io())
            .map(tick -> makeHttpRequest())
            .map(response -> {
                if (response.isSuccessful()) {
                    ArrayResponse<TagEntity> res = ArrayResponse.fromJson(response.body().string(), TagEntity.class);
                    response.close();
                    return res;
                } else {

...
/*
import com.fireback.CurrencyEntity;
*/
...
public class PostCurrency {
    public static String Url  = FirebackConfig.getInstance().BuildUrl("/currency");
    public Single<SingleResponse<CurrencyEntity>> post(CurrencyEntity dto) {
        return Single.fromCallable(() -> makeHttpPostRequest(dto))
                .subscribeOn(Schedulers.io());
    }
    private SingleResponse<CurrencyEntity> makeHttpPostRequest(CurrencyEntity dto) throws ResponseErrorException {
        ...
        try (Response response = client.newCall(request).execute()) {
            if (response.isSuccessful()) {
                SingleResponse<CurrencyEntity> res = SingleResponse.fromJson(response.body().string(), CurrencyEntity.class);
                response.close();
                return res;
            } else {

...
type paymentConfigActionsSig struct {
	...
	Query          func(query fireback.QueryDSL) ([]*PaymentConfigEntity, *fireback.QueryResultMeta, *fireback.IError)
}
...
func PaymentConfigActionQueryFn(query fireback.QueryDSL) ([]*PaymentConfigEntity, *fireback.QueryResultMeta, *fireback.IError) {
	refl := reflect.ValueOf(&PaymentConfigEntity{})
...
func (x *PaymentConfigEntity) FromCli(c *cli.Command) *PaymentConfigEntity {
	return CastPaymentConfigFromCli(c)
}
func CastPaymentConfigFromCli(c *cli.Command) *PaymentConfigEntity {
...
func PaymentConfigsActionQueryString(keyword string, page int) ([]string, *fireback.QueryResultMeta, *fireback.IError) {
	searchFields := []string{
		`unique_id %"{keyword}"%`,
		`name %"{keyword}"%`,
	}
...
var PaymentConfigImportExportCommands = []cli.Command{
	{
		...
		Action: func(ctx context.Context, c *cli.Command) error {
			data := fireback.ContentImport[PaymentConfigEntity]{}
	...
	cli.Command{
		...
		Action: func(ctx context.Context, c *cli.Command) error {
			if seeders, err := fireback.GetSeederFilenames(paymentConfigSeedersFs, ""); err != nil {
				return err
			} else {
				if len(seeders) == 0 {

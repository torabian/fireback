...
// WorkspaceConfigAwareDeleteActionRaw registers a raw Gin route for the WorkspaceConfigAwareDeleteAction action.
// This gives the developer full control over middleware, handlers, and response handling.
func WorkspaceConfigAwareDeleteActionRaw(r *gin.Engine, handlers ...gin.HandlerFunc) {
	meta := WorkspaceConfigAwareDeleteActionMeta()
	r.Handle(meta.Method, meta.URL, handlers...)
}

// WorkspaceConfigAwareDeleteActionHandler returns the HTTP method, route URL, and a typed Gin handler for the WorkspaceConfigAwareDeleteAction action.
// Developers implement their business logic as a function that receives a typed request object
// and returns either an *ActionResponse or nil. JSON marshalling, headers, and errors are handled automatically.
func WorkspaceConfigAwareDeleteActionHandler(
...
// WorkspaceConfigAwareDeleteActionGin is a high-level convenience wrapper around WorkspaceConfigAwareDeleteActionHandler.
// It automatically constructs and registers the typed route on the Gin engine.
// Use this when you don't need custom middleware or route grouping.
func WorkspaceConfigAwareDeleteActionGin(r gin.IRoutes, handler func(c WorkspaceConfigAwareDeleteActionRequest) (*WorkspaceConfigAwareDeleteActionResponse, error)) {
	method, url, h := WorkspaceConfigAwareDeleteActionHandler(handler)
	r.Handle(method, url, h)
}
func (x WorkspaceConfigAwareDeleteActionRequest) IsGin() bool {
	if x.GinCtx == nil {
		return false
	}
...
func WorkspaceConfigAwareDeleteActionQueryFromGin(c *gin.Context) WorkspaceConfigAwareDeleteActionQuery {
	return WorkspaceConfigAwareDeleteActionQueryFromString(c.Request.URL.RawQuery)
}

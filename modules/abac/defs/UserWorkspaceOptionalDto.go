...
func GetUserWorkspaceOptionalDtoCliFlags(prefix string) []emigo.CliFlag {
...
func CastUserWorkspaceOptionalDtoFromCli(c emigo.CliCastable) UserWorkspaceOptionalDto {
	data := UserWorkspaceOptionalDto{}
	...
	if c.IsSet("user-permissions") {
		emigo.ParseNullable(c.String("user-permissions"), &data.UserPermissions)
	}
	if c.IsSet("role-permission") {
		emigo.ParseNullable(c.String("role-permission"), &data.RolePermission)
	}
	if c.IsSet("workspace-permissions") {
		emigo.ParseNullable(c.String("workspace-permissions"), &data.WorkspacePermissions)
	}

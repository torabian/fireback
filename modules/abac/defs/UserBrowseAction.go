...
// UserBrowseActionRaw registers a raw Gin route for the UserBrowseAction action.
// This gives the developer full control over middleware, handlers, and response handling.
func UserBrowseActionRaw(r *gin.Engine, handlers ...gin.HandlerFunc) {
	meta := UserBrowseActionMeta()
	r.Handle(meta.Method, meta.URL, handlers...)
}

// UserBrowseActionHandler returns the HTTP method, route URL, and a typed Gin handler for the UserBrowseAction action.
// Developers implement their business logic as a function that receives a typed request object
// and returns either an *ActionResponse or nil. JSON marshalling, headers, and errors are handled automatically.
func UserBrowseActionHandler(
...
// UserBrowseActionGin is a high-level convenience wrapper around UserBrowseActionHandler.
// It automatically constructs and registers the typed route on the Gin engine.
// Use this when you don't need custom middleware or route grouping.
func UserBrowseActionGin(r gin.IRoutes, handler func(c UserBrowseActionRequest) (*UserBrowseActionResponse, error)) {
	method, url, h := UserBrowseActionHandler(handler)
	r.Handle(method, url, h)
}
func (x UserBrowseActionRequest) IsGin() bool {
	if x.GinCtx == nil {
		return false
	}
...
func UserBrowseActionQueryFromGin(c *gin.Context) UserBrowseActionQuery {
	return UserBrowseActionQueryFromString(c.Request.URL.RawQuery)
}
func GetUserBrowseActionQueryCliFlags(prefix string) []emigo.CliFlag {
...
// UserBrowseActionQueryFromCli extracts and casts query parameters the same way
// UserBrowseActionQueryFromString does, but reads them off urfave v3 CLI flags instead
// of a raw query string. The underlying url.Values (as returned by .Values()) is filled
// in using each field's real name, so code consuming req.QueryParams behaves the same
// whether the request came from HTTP or from the CLI.
func UserBrowseActionQueryFromCli(c *cli.Command) UserBrowseActionQuery {
	data := UserBrowseActionQuery{}
	values := url.Values{}
...
func (x UserBrowseActionRequest) IsCli() bool {
	if x.CliCtx == nil {
		return false
	}
...
// UserBrowseActionCliFlags returns every flag (request body, path parameters,
// query parameters and typed headers) the UserBrowseAction action can bind from
// urfave v3, plus a generic repeatable --header/-H flag for anything not covered by a
// typed header.
func UserBrowseActionCliFlags() []cli.Flag {
	flags := []cli.Flag{
		&cli.StringSliceFlag{
			Name:    "header",
			Aliases: []string{"H"},
	...
	flags = append(flags, emigo.CastEmiFlagToUrfave(GetUserBrowseActionQueryCliFlags(""))...)
	return flags
}

// UserBrowseActionCliHandler builds a full *cli.Command for the
// UserBrowseAction action: it wires body, path parameters, query parameters and
// headers from urfave v3 CLI flags into a UserBrowseActionRequest the same way
// UserBrowseActionHandler (Gin) and UserBrowseActionHttpHandler (net/http)
// do from their own transports, then prints the JSON response (or returns the error) so
// urfave reports the right exit code.
func UserBrowseActionCliHandler(
	handler func(c UserBrowseActionRequest) (*UserBrowseActionResponse, error),
) *cli.Command {
	meta := UserBrowseActionMeta()
	cmd := &cli.Command{
		...
		Flags: UserBrowseActionCliFlags(),
	}
	...
	cmd.Action = func(ctx context.Context, c *cli.Command) error {
		...
		req.QueryParams = UserBrowseActionQueryFromCli(c).Values()
		return emigo.HandleActionInCli(handler(req))
	}
	return cmd
}

// UserBrowseActionCli is a high-level convenience wrapper around
// UserBrowseActionCliHandler. It registers the generated command as a subcommand
// of an existing urfave v3 *cli.Command, the same way UserBrowseActionGin
// registers a route on a Gin engine.
func UserBrowseActionCli(
	app *cli.Command,
	handler func(c UserBrowseActionRequest) (*UserBrowseActionResponse, error),
) {
	app.Commands = append(app.Commands, UserBrowseActionCliHandler(handler))
}

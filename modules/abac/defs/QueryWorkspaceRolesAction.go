...
func GetQueryWorkspaceRolesActionResCliFlags(prefix string) []emigo.CliFlag {
	return []emigo.CliFlag{
		{
...
func CastQueryWorkspaceRolesActionResFromCli(c emigo.CliCastable) QueryWorkspaceRolesActionRes {
	data := QueryWorkspaceRolesActionRes{}
	...
	if c.IsSet("capabilities-list-id") {
		emigo.InflatePossibleSlice(c.String("capabilities-list-id"), &data.CapabilitiesListId)
	}
	return data
}
...
// QueryWorkspaceRolesActionRaw registers a raw Gin route for the QueryWorkspaceRolesAction action.
// This gives the developer full control over middleware, handlers, and response handling.
func QueryWorkspaceRolesActionRaw(r *gin.Engine, handlers ...gin.HandlerFunc) {
	meta := QueryWorkspaceRolesActionMeta()
	r.Handle(meta.Method, meta.URL, handlers...)
}

// QueryWorkspaceRolesActionHandler returns the HTTP method, route URL, and a typed Gin handler for the QueryWorkspaceRolesAction action.
// Developers implement their business logic as a function that receives a typed request object
// and returns either an *ActionResponse or nil. JSON marshalling, headers, and errors are handled automatically.
func QueryWorkspaceRolesActionHandler(
...
// QueryWorkspaceRolesActionGin is a high-level convenience wrapper around QueryWorkspaceRolesActionHandler.
// It automatically constructs and registers the typed route on the Gin engine.
// Use this when you don't need custom middleware or route grouping.
func QueryWorkspaceRolesActionGin(r gin.IRoutes, handler func(c QueryWorkspaceRolesActionRequest) (*QueryWorkspaceRolesActionResponse, error)) {
	method, url, h := QueryWorkspaceRolesActionHandler(handler)
	r.Handle(method, url, h)
}
func (x QueryWorkspaceRolesActionRequest) IsGin() bool {
	if x.GinCtx == nil {
		return false
	}
...
func QueryWorkspaceRolesActionQueryFromGin(c *gin.Context) QueryWorkspaceRolesActionQuery {
	return QueryWorkspaceRolesActionQueryFromString(c.Request.URL.RawQuery)
}

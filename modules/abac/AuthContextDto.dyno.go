...
func CastAuthContextFromCli(c *cli.Context) *AuthContextDto {
	...
	if c.IsSet("security-id") {
		template.SecurityId = workspaces.NewStringAutoNull(c.String("security-id"))
	}
	return template
}
...
type AuthContextDto struct {
	...
	Security     *workspaces.SecurityModel   `json:"security" yaml:"security"    gorm:"foreignKey:SecurityId;references:UniqueId"      `
	SecurityId   workspaces.String           `json:"securityId" yaml:"securityId"`
	Capabilities []workspaces.PermissionInfo `json:"capabilities" yaml:"capabilities"        `
}

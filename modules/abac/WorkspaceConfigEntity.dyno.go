...
type workspaceConfigActionsSig struct {
	...
	Query          func(query fireback.QueryDSL) ([]*WorkspaceConfigEntity, *fireback.QueryResultMeta, *fireback.IError)
}
...
func WorkspaceConfigActionQueryFn(query fireback.QueryDSL) ([]*WorkspaceConfigEntity, *fireback.QueryResultMeta, *fireback.IError) {
	refl := reflect.ValueOf(&WorkspaceConfigEntity{})
...
func (x *WorkspaceConfigEntity) FromCli(c *cli.Command) *WorkspaceConfigEntity {
	return CastWorkspaceConfigFromCli(c)
}
func CastWorkspaceConfigFromCli(c *cli.Command) *WorkspaceConfigEntity {
...
func WorkspaceConfigsActionQueryString(keyword string, page int) ([]string, *fireback.QueryResultMeta, *fireback.IError) {
	searchFields := []string{
		`unique_id %"{keyword}"%`,
		`name %"{keyword}"%`,
	}
...
var WorkspaceConfigImportExportCommands = []*cli.Command{
	{
		...
		Action: func(ctx context.Context, c *cli.Command) error {
			data := fireback.ContentImport[WorkspaceConfigEntity]{}
	...
	},
	{
		...
		Action: func(ctx context.Context, c *cli.Command) error {
			if seeders, err := fireback.GetSeederFilenames(workspaceConfigSeedersFs, ""); err != nil {
				return err
			} else {
				if len(seeders) == 0 {
...
var WORKSPACE_CONFIG_ACTION_PATCH = fireback.Module3Action{
	...
	Description: "Update the WorkspaceConfig entity by unique id",
	CliName:     "update",
	CliAction: func(c *cli.Command, security *fireback.SecurityModel) error {

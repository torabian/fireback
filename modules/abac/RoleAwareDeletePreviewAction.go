...
func RoleAwareDeletePreviewActionMeta() struct {
	...
	return struct {
		...
		CliName:     "role-aware-delete-preview-action",
		CliShort:    "role-dp",
...
func GetRoleAwareDeletePreviewActionResCliFlags(prefix string) []emigo.CliFlag {
...
func CastRoleAwareDeletePreviewActionResFromCli(c emigo.CliCastable) RoleAwareDeletePreviewActionRes {
	data := RoleAwareDeletePreviewActionRes{}
	if c.IsSet("message") {
		data.Message = c.String("message")
	}
	if c.IsSet("affected") {
		data.Affected = emigo.CapturePossibleArray(CastRoleAwareDeletePreviewActionResAffectedFromCli, "affected", c)
	}
	return data
}
func GetRoleAwareDeletePreviewActionResAffectedCliFlags(prefix string) []emigo.CliFlag {
	return []emigo.CliFlag{
		{
...
func CastRoleAwareDeletePreviewActionResAffectedFromCli(c emigo.CliCastable) RoleAwareDeletePreviewActionResAffected {
	data := RoleAwareDeletePreviewActionResAffected{}
	if c.IsSet("relation") {
...
// RoleAwareDeletePreviewActionRaw registers a raw Gin route for the RoleAwareDeletePreviewAction action.
// This gives the developer full control over middleware, handlers, and response handling.
func RoleAwareDeletePreviewActionRaw(r *gin.Engine, handlers ...gin.HandlerFunc) {
	meta := RoleAwareDeletePreviewActionMeta()
	r.Handle(meta.Method, meta.URL, handlers...)
}

// RoleAwareDeletePreviewActionHandler returns the HTTP method, route URL, and a typed Gin handler for the RoleAwareDeletePreviewAction action.
// Developers implement their business logic as a function that receives a typed request object
// and returns either an *ActionResponse or nil. JSON marshalling, headers, and errors are handled automatically.
func RoleAwareDeletePreviewActionHandler(
...
// RoleAwareDeletePreviewActionGin is a high-level convenience wrapper around RoleAwareDeletePreviewActionHandler.
// It automatically constructs and registers the typed route on the Gin engine.
// Use this when you don't need custom middleware or route grouping.
func RoleAwareDeletePreviewActionGin(r gin.IRoutes, handler func(c RoleAwareDeletePreviewActionRequest) (*RoleAwareDeletePreviewActionResponse, error)) {
	method, url, h := RoleAwareDeletePreviewActionHandler(handler)
	r.Handle(method, url, h)
}
func (x RoleAwareDeletePreviewActionRequest) IsGin() bool {
	if x.GinCtx == nil {
		return false
	}
...
func RoleAwareDeletePreviewActionQueryFromGin(c *gin.Context) RoleAwareDeletePreviewActionQuery {
	return RoleAwareDeletePreviewActionQueryFromString(c.Request.URL.RawQuery)
}

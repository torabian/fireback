...
func GetUserEntityCliFlags(prefix string) []emigo.CliFlag {
	return []emigo.CliFlag{
		...
		{
			Name: prefix + "primary-address-row",
			Type: "complex",
		},
	}
}
func CastUserEntityFromCli(c emigo.CliCastable) UserEntity {
	...
	if c.IsSet("primary-address-row") {
		if u, ok := any(&data.PrimaryAddressRow).(encoding.TextUnmarshaler); ok {
			u.UnmarshalText([]byte(c.String("primary-address-row")))
		}
	}
	return data
}
func GetUserEntityPrimaryAddressCliFlags(prefix string) []emigo.CliFlag {
	return []emigo.CliFlag{
		{
...
func CastUserEntityPrimaryAddressFromCli(c emigo.CliCastable) UserEntityPrimaryAddress {
	data := UserEntityPrimaryAddress{}

...
type TestnullModuleConfig struct {
...
func TestnullModuleSetup(cfg *TestnullModuleConfig) *workspaces.ModuleProvider {
	module := &workspaces.ModuleProvider{
		...
		ActionsBundle: GetTestnullActionsBundle(),
...
type Example struct {
	IsNull *null.Bool `json:"isNull"`
}

func init() {
	TestNullActionImp = func(req *TestNullActionReqDto, q workspaces.QueryDSL) (string, *workspaces.IError) {

		workspaces.GetDbRef().Debug().Model(&SamplerEntity{}).Where(&SamplerEntity{IsSet: req.IsSet}).Find(&SamplerEntity{IsSet: req.IsSet})
		fmt.Println(req.IsSet)
		return "okay", nil
	}
}

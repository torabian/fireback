...
var PRODUCTTAG_EVENT_DELETED = "productTag.deleted"
var PRODUCTTAG_EVENTS = []string{
	PRODUCTTAG_EVENT_CREATED,
...
func ProductTagMockEntity() *ProductTagEntity {
	stringHolder := "~"
...
func ProductTagActionSeeder(query workspaces.QueryDSL, count int) {
	successInsert := 0
	failureInsert := 0
	bar := progressbar.Default(int64(count))
	for i := 1; i <= count; i++ {
		entity := ProductTagMockEntity()
		_, err := ProductTagActionCreate(entity, query)
		if err == nil {
			successInsert++
		} else {
			fmt.Println(err)
...
func ProductTagPolyglotCreateHandler(dto *ProductTagEntity, query workspaces.QueryDSL) {
	if dto == nil {
		return
	}
...
func ProductTagActionBatchCreateFn(dtos []*ProductTagEntity, query workspaces.QueryDSL) ([]*ProductTagEntity, *workspaces.IError) {
	if dtos != nil && len(dtos) > 0 {
		...
		for _, item := range dtos {
			s, err := ProductTagActionCreateFn(item, query)
			if err != nil {
				return nil, err
			}
...
func ProductTagActionCreateFn(dto *ProductTagEntity, query workspaces.QueryDSL) (*ProductTagEntity, *workspaces.IError) {
	// 1. Validate always
	if iError := ProductTagValidator(dto, false); iError != nil {
		return nil, iError
	}
	// 1.5 Sanitize the content coming of the front-end
	ProductTagEntityPreSanitize(dto, query)
	// 2. Append the necessary information about user, workspace
	ProductTagEntityBeforeCreateAppend(dto, query)
	// 3. Append the necessary translations, even if english
	ProductTagPolyglotCreateHandler(dto, query)
	// 3.5. Create other entities if we want select from them
	ProductTagRelationContentCreate(dto, query)
	// 4. Create the entity
...
var ProductTagWipeCmd cli.Command = cli.Command{
	...
	Action: func(c *cli.Context) error {
		...
		count, _ := ProductTagActionWipeClean(query)
...
func ProductTagActionExport(
	...
	return workspaces.YamlExporterChannel[ProductTagEntity](query, ProductTagActionQuery, ProductTagPreloadRelations)
}
func ProductTagActionExportT(
	query workspaces.QueryDSL,
) (chan []interface{}, *workspaces.IError) {
	return workspaces.YamlExporterChannelT[ProductTagEntity](query, ProductTagActionQuery, ProductTagPreloadRelations)
}
func ProductTagActionImport(
	dto interface{}, query workspaces.QueryDSL,
) *workspaces.IError {
	var json = jsoniter.ConfigCompatibleWithStandardLibrary
	var content ProductTagEntity
...
var ProductTagImportExportCommands = []cli.Command{
	{
		...
		Action: func(c *cli.Context) error {
			query := workspaces.CommonCliQueryDSLBuilder(c)
			ProductTagActionSeeder(query, c.Int("count"))
			return nil
		},
	},
	{
		...
		Action: func(c *cli.Context) error {
			...
			ProductTagActionSeederInit(f, c.String("file"), c.String("format"))
			return nil
		},
	},
	{
		...
		Flags: []cli.Flag{
			&cli.StringFlag{
				...
				Value: "product-tag-seeder-product-tag.yml",
				// Uncomment before publish, they need to specify
				// Required: true,
			},
			&cli.StringFlag{
		...
		Usage: "Reads a yaml file containing an array of product-tags, you can run this to validate if your import file is correct, and how it would look like after import",
		Action: func(c *cli.Context) error {
			data := &[]ProductTagEntity{}
...
var PERM_ROOT_PRODUCTTAG_QUERY = "root/producttag/query"
var PERM_ROOT_PRODUCTTAG = "root/producttag"
var ALL_PRODUCTTAG_PERMISSIONS = []string{
	PERM_ROOT_PRODUCTTAG_DELETE,
	PERM_ROOT_PRODUCTTAG_CREATE,
	PERM_ROOT_PRODUCTTAG_UPDATE,
	PERM_ROOT_PRODUCTTAG_QUERY,
	PERM_ROOT_PRODUCTTAG,
}
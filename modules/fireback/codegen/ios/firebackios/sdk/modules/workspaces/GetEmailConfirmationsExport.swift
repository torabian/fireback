...
import Combine
import SwiftUI
func GetEmailConfirmationsExportFetcher() -> AnyPublisher<ArrayResponse<EmailConfirmationEntity>, Error> {
  var prefix = ""
  if let api_url = ProcessInfo.processInfo.environment["api_url"] {
    prefix = api_url
  }
  let url = URL(string: prefix + "/email-confirmations/export")!
  var request = URLRequest(url: url)
    ...
    return URLSession.shared
        ...
        .map(\.data)
        .decode(type: ArrayResponse<EmailConfirmationEntity>.self, decoder: JSONDecoder())
        .receive(on: DispatchQueue.main)
        .eraseToAnyPublisher()
}
/*
I have commented this for now, because this is not returning correctly, as well as the function above is good enough
func GetEmailConfirmationsExport() -> Promise<[EmailConfirmationEntity]?> {

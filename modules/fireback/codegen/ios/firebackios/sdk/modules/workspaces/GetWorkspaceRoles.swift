...
import Combine
import SwiftUI
func GetWorkspaceRolesFetcher() -> AnyPublisher<ArrayResponse<WorkspaceRoleEntity>, Error> {
  var prefix = ""
  if let api_url = ProcessInfo.processInfo.environment["api_url"] {
    prefix = api_url
  }
  let url = URL(string: prefix + "/workspace-roles")!
...
func GetWorkspaceRoles() -> Promise<[WorkspaceRoleEntity]?> {

...
import { execApiFn, IResponseList } from "../../core/http-tools";
    import {
        InvoiceEntity,
    } from "../payment/InvoiceEntity"
// Fireback gives the option to have typesafe query string
export type GetInvoicesExportQs = {
}
export type GetInvoicesExportUrlParams = {
}
export type GetInvoicesExportHeaders = {
}
export function useGetInvoicesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  unauthorized,
  optionFn
}: UseRemoteQuery & {query?: GetInvoicesExportQs}) {
  ...
  const url = "/invoices/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<InvoiceEntity>, any>(["*payment.InvoiceEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<InvoiceEntity> = query$.data?.data?.items || [];
  return { 
    query: query$,
    items,
    keyExtractor: (item: InvoiceEntity) => item.uniqueId,
  };
}
useGetInvoicesExport.UKEY = "*payment.InvoiceEntity"
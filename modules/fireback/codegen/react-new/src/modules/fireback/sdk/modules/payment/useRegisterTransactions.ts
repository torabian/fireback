...
import {
  ...
  queryBeforeSend,
    ...
    import {
        RegisterTransactionActionReqDto,
    } from "../payment/PaymentActionsDto"
export function useRegisterTransactions(
  ...
  const url = "/register-transactions".substr(1);
  let computedUrl = `${url}?${new URLSearchParams(
    queryBeforeSend(query)
  ).toString()}`;
  let completeRouteUrls = true;
  // Attach the details of the request to the fn
  const fn = (body: any) => rpcFn("", computedUrl, body);
  const mutation = useMutation<
    IResponse<any>,
    IResponse<any>,
    Partial<RegisterTransactionActionReqDto>
  >(fn);
  ...
  const submit = (
    values: Partial<RegisterTransactionActionReqDto>,

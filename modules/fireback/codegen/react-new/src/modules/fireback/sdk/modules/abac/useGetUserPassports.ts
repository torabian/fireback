...
export type GetUserPassportsQs = {
}
export type GetUserPassportsUrlParams = {
}
export type GetUserPassportsHeaders = {
}
export function useGetUserPassports({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  unauthorized,
  optionFn
}: UseRemoteQuery & {query?: GetUserPassportsQs}) {
  ...
  const url = "/user/passports".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<UserPassportsActionResDto>, any>(["*abac.UserPassportsActionResDto", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<UserPassportsActionResDto> = query$.data?.data?.items || [];
  return { 
    query: query$,
    items,
...
useGetUserPassports.UKEY = "*abac.UserPassportsActionResDto"
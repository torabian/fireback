...
export type GetUrwQueryQs = {
}
export type GetUrwQueryUrlParams = {
}
export type GetUrwQueryHeaders = {
}
export function useGetUrwQuery({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  unauthorized,
  optionFn
}: UseRemoteQuery & {query?: GetUrwQueryQs}) {
  ...
  const query$ = useQuery<any, any, IResponseList<QueryUserRoleWorkspacesActionResDto>, any>(["*abac.QueryUserRoleWorkspacesActionResDto", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<QueryUserRoleWorkspacesActionResDto> = query$.data?.data?.items || [];
  return { 
    query: query$,
    items,
...
useGetUrwQuery.UKEY = "*abac.QueryUserRoleWorkspacesActionResDto"
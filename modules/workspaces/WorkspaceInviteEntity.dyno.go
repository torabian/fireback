...
func WorkspaceInviteEntityStream(q QueryDSL) (chan []*WorkspaceInviteEntity, *QueryResultMeta, error) {
	...
	q.ItemsPerPage = 50
	q.StartIndex = 0
	_, qrm, err := WorkspaceInviteActionQuery(q)
	if err != nil {
		return nil, nil, err
	}
	go func() {
...
func (x *WorkspaceInviteEntity) Seeder() string {
	obj := WorkspaceInviteActionSeederInit()
	v, _ := json.MarshalIndent(obj, "", "  ")
	return string(v)
}
func WorkspaceInviteActionSeederInit() *WorkspaceInviteEntity {
	tildaRef := "~"
	_ = tildaRef
	entity := &WorkspaceInviteEntity{
...
func WorkspaceInvitesActionQueryString(keyword string, page int) ([]string, *QueryResultMeta, error) {
	...
	items, meta, err := WorkspaceInviteActionQuery(query)
	stringItems := []string{}
	for _, item := range items {
		label := m(item)
...
var WorkspaceInviteImportExportCommands = []cli.Command{
	...
	},
	{
		...
		Action: func(c *cli.Context) error {
			seed := WorkspaceInviteActionSeederInit()

...
export function useGetShoppingCartsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/shopping-carts/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<ShoppingCartEntity>, any>(["*shop.ShoppingCartEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<ShoppingCartEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetShoppingCartsExport.UKEY = "*shop.ShoppingCartEntity"
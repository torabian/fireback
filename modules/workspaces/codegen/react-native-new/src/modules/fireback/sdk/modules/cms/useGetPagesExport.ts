...
export function useGetPagesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/pages/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<PageEntity>, any>(["*cms.PageEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<PageEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetPagesExport.UKEY = "*cms.PageEntity"
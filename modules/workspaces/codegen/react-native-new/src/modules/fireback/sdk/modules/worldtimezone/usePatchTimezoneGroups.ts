...
export function usePatchTimezoneGroups({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  query = query || {}
  ...
  const fnUpdater: any = (
    data: PossibleStoreData<TimezoneGroupEntity>,
    response: IResponse<BulkRecordRequest<TimezoneGroupEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<TimezoneGroupEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[worldtimezone.TimezoneGroupEntity]", (data: any) =>

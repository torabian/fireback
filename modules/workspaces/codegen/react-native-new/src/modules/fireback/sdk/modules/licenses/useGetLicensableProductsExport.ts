...
export function useGetLicensableProductsExport({
  ...
  const query$ = useQuery<any, any, IResponseList<LicensableProductEntity>, any>(["*licenses.LicensableProductEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<LicensableProductEntity> = query$.data?.data?.items || [];

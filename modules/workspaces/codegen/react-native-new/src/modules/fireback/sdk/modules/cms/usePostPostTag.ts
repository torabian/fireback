...
export function usePostPostTag(props?: UseRemoteQuery) {
  ...
  const fnUpdater = (
    data: IResponseList<PostTagEntity> | undefined,
    item: IResponse<PostTagEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<PostTagEntity>) {
          queryClient?.setQueryData<IResponseList<PostTagEntity>>(
            "*cms.PostTagEntity",

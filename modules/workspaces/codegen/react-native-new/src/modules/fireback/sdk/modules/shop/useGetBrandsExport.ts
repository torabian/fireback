...
export function useGetBrandsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/brands/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<BrandEntity>, any>(["*shop.BrandEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<BrandEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetBrandsExport.UKEY = "*shop.BrandEntity"
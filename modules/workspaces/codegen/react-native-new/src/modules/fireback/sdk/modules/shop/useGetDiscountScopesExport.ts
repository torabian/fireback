...
export function useGetDiscountScopesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/discount-scopes/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<DiscountScopeEntity>, any>(["*shop.DiscountScopeEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<DiscountScopeEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetDiscountScopesExport.UKEY = "*shop.DiscountScopeEntity"
...
export function useGetUsersInvitations({
  ...
  const query$ = useQuery<any, any, IResponseList<UserInvitationsActionResDto>, any>(["*workspaces.UserInvitationsActionResDto", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<UserInvitationsActionResDto> = query$.data?.data?.items || [];
  return { 
    query: query$,
    items,
    keyExtractor: (item: UserInvitationsActionResDto) => item.uniqueId,
  };
}
useGetUsersInvitations.UKEY = "*workspaces.UserInvitationsActionResDto"
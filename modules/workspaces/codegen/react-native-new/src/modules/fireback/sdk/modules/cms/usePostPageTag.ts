...
export function usePostPageTag(props?: UseRemoteQuery) {
  ...
  const fnUpdater = (
    data: IResponseList<PageTagEntity> | undefined,
    item: IResponse<PageTagEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<PageTagEntity>) {
          queryClient?.setQueryData<IResponseList<PageTagEntity>>(
            "*cms.PageTagEntity",

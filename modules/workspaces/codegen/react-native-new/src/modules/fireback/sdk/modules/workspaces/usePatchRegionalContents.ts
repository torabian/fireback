...
export function usePatchRegionalContents({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  query = query || {}
  ...
  const url = "/regional-contents".substr(1);
  ...
  const fnUpdater: any = (
    data: PossibleStoreData<RegionalContentEntity>,
    response: IResponse<BulkRecordRequest<RegionalContentEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<RegionalContentEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[workspaces.RegionalContentEntity]", (data: any) =>

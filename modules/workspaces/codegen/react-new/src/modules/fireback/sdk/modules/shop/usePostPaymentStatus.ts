...
export function usePostPaymentStatus(props?: UseRemoteQuery) {
  ...
  const url = "/payment-status".substr(1);
  ...
  const fnUpdater = (
    data: IResponseList<PaymentStatusEntity> | undefined,
    item: IResponse<PaymentStatusEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<PaymentStatusEntity>) {
          queryClient?.setQueryData<IResponseList<PaymentStatusEntity>>(
            "*shop.PaymentStatusEntity",

...
export function useGetPostTagsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/post-tags/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<PostTagEntity>, any>(["*cms.PostTagEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<PostTagEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetPostTagsExport.UKEY = "*cms.PostTagEntity"
...
export function useGetPostCategoriesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/post-categories/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<PostCategoryEntity>, any>(["*cms.PostCategoryEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<PostCategoryEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetPostCategoriesExport.UKEY = "*cms.PostCategoryEntity"
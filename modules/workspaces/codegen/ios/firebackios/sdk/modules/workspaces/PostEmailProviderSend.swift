import Promises
func PostEmailProviderSend(dto: SendEmailWithProviderActionReqDto) -> Promise<SendEmailWithProviderActionResDto?> {
    return Promise<SendEmailWithProviderActionResDto?>(on: .main) { fulfill, reject in
  var prefix = ""
  if let api_url = ProcessInfo.processInfo.environment["api_url"] {
    prefix = api_url
  }
  let url = URL(string: prefix + "/emailProvider/send")!
  var request = URLRequest(url: url)
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            if let data = data {
                ...
                do {
                    let result = try decoder.decode(SingleResponse<SendEmailWithProviderActionResDto>.self, from: data)
                    if result.error != nil {
                        reject(result.error!)
                    } else {
                        fulfill(result.data)

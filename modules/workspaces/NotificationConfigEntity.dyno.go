  ...
  func NotificationConfigActionUpdateFn(query QueryDSL, fields *NotificationConfigEntity) (*NotificationConfigEntity, *IError) {
    ...
    if query.Tx == nil {
      ...
      vf := dbref.Transaction(func(tx *gorm.DB) error {
        dbref = tx
        _, err := NotificationConfigUpdateExec(dbref, query, fields)
        if err == nil {
          return nil
        } else {
          return err
...
var NotificationConfigCommonCliFlagsOptional = []cli.Flag{
  ...
  var NotificationConfigCreateCmd cli.Command = cli.Command{
    Name:    "create",
    Aliases: []string{"c"},
...
var NotificationConfigImportExportCommands = []cli.Command{
	...
	},
	{
		...
		Action: func(c *cli.Context) error {
			...
			NotificationConfigActionSeederInit(f, c.String("file"), c.String("format"))
			return nil
		},
	},
	{
    ...
    var NotificationConfigCliCommands []cli.Command = []cli.Command{
      GetCommonQuery(NotificationConfigActionQuery),

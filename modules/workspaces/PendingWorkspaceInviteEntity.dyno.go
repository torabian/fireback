...
func PendingWorkspaceInviteActionUpdateFn(query QueryDSL, fields *PendingWorkspaceInviteEntity) (*PendingWorkspaceInviteEntity, *IError) {
	...
	if query.Tx == nil {
		...
		vf := dbref.Transaction(func(tx *gorm.DB) error {
			dbref = tx
			_, err := PendingWorkspaceInviteUpdateExec(dbref, query, fields)
			if err == nil {
				return nil
			} else {
				return err
...
var PendingWorkspaceInviteCreateCmd cli.Command = cli.Command{
	Name:    "create",
	Aliases: []string{"c"},
...
var PendingWorkspaceInviteImportExportCommands = []cli.Command{
	...
	},
	{
		...
		Action: func(c *cli.Context) error {
			...
			PendingWorkspaceInviteActionSeederInit(f, c.String("file"), c.String("format"))
			return nil
		},
	},
	{
...
var PendingWorkspaceInviteCliCommands []cli.Command = []cli.Command{
	GetCommonQuery(PendingWorkspaceInviteActionQuery),

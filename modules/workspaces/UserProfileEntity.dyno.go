...
func UserProfileActionUpdateFn(query QueryDSL, fields *UserProfileEntity) (*UserProfileEntity, *IError) {
	...
	if query.Tx == nil {
		...
		vf := dbref.Transaction(func(tx *gorm.DB) error {
			dbref = tx
			_, err := UserProfileUpdateExec(dbref, query, fields)
			if err == nil {
				return nil
			} else {
				return err
...
var UserProfileCreateCmd cli.Command = cli.Command{
	Name:    "create",
	Aliases: []string{"c"},
...
var UserProfileImportExportCommands = []cli.Command{
	...
	},
	{
		...
		Action: func(c *cli.Context) error {
			...
			UserProfileActionSeederInit(f, c.String("file"), c.String("format"))
			return nil
		},
	},
	{
...
var UserProfileCliCommands []cli.Command = []cli.Command{
	GetCommonQuery(UserProfileActionQuery),

...
func RoleActionUpdateFn(query QueryDSL, fields *RoleEntity) (*RoleEntity, *IError) {
	...
	if query.Tx == nil {
		...
		vf := dbref.Transaction(func(tx *gorm.DB) error {
			dbref = tx
			_, err := RoleUpdateExec(dbref, query, fields)
			if err == nil {
				return nil
			} else {
				return err
...
var RoleCreateCmd cli.Command = cli.Command{
	Name:    "create",
	Aliases: []string{"c"},
...
var RoleImportExportCommands = []cli.Command{
	...
	},
	{
		...
		Action: func(c *cli.Context) error {
			...
			RoleActionSeederInit(f, c.String("file"), c.String("format"))
			return nil
		},
	},
	{
...
var RoleCliCommands []cli.Command = []cli.Command{
	GetCommonQuery(RoleActionQuery),

...
func PersonUpdateExec(dbref *gorm.DB, query QueryDSL, fields *PersonEntity) (*PersonEntity, *IError) {
	...
	cond2 := &PersonEntity{UniqueId: uniqueId}
	q := dbref.
		Where(cond2).
		FirstOrCreate(&item)
...
func CastPersonFromCli(c *cli.Context) *PersonEntity {
	...
	if c.IsSet("gender") {
		template.Gender = c.String("gender")
	}
	if c.IsSet("title") {
...
func PeopleActionQueryString(keyword string, page int) ([]string, *QueryResultMeta, error) {
	searchFields := []string{
		`unique_id %"{keyword}"%`,
		`name %"{keyword}"%`,
	}
	m := func(item *PersonEntity) string {
		label := item.UniqueId
		// if item.Name != nil {
		// 	label += " >>> " + *item.Name
		// }
...
var PERM_ROOT_PERSON = PermissionInfo{
	CompleteKey: "root.manage.workspaces.person.*",
	Name:        "Entire person actions (*)",
	Description: "",
}
var PERM_ROOT_PERSON_DELETE = PermissionInfo{
	CompleteKey: "root.manage.workspaces.person.delete",
	Name:        "Delete person",
	Description: "",
}
var PERM_ROOT_PERSON_CREATE = PermissionInfo{
	CompleteKey: "root.manage.workspaces.person.create",
	Name:        "Create person",
	Description: "",
}
var PERM_ROOT_PERSON_UPDATE = PermissionInfo{
	CompleteKey: "root.manage.workspaces.person.update",
	Name:        "Update person",
	Description: "",
}
var PERM_ROOT_PERSON_QUERY = PermissionInfo{
	CompleteKey: "root.manage.workspaces.person.query",
	Name:        "Query person",
	Description: "",
}

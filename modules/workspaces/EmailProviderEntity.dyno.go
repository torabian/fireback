  ...
  func EmailProviderActionUpdateFn(query QueryDSL, fields *EmailProviderEntity) (*EmailProviderEntity, *IError) {
    ...
    if query.Tx == nil {
      ...
      vf := dbref.Transaction(func(tx *gorm.DB) error {
        dbref = tx
        _, err := EmailProviderUpdateExec(dbref, query, fields)
        if err == nil {
          return nil
        } else {
          return err
...
var EmailProviderCommonCliFlagsOptional = []cli.Flag{
  ...
  var EmailProviderCreateCmd cli.Command = cli.Command{
    Name:    "create",
    Aliases: []string{"c"},
...
var EmailProviderImportExportCommands = []cli.Command{
	...
	},
	{
		...
		Action: func(c *cli.Context) error {
			...
			EmailProviderActionSeederInit(f, c.String("file"), c.String("format"))
			return nil
		},
	},
	{
    ...
    var EmailProviderCliCommands []cli.Command = []cli.Command{
      GetCommonQuery(EmailProviderActionQuery),

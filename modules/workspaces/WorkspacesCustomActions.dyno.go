...
var ConfirmClassicPassportTotpCommonCliFlagsOptional = []cli.Flag{
	&cli.StringFlag{
		Name:     "value",
		Required: true,
		Usage:    `Passport value, email or phone number which is already successfully registered.`,
	},
	&cli.StringFlag{
		Name:     "password",
		Required: true,
		Usage:    `Password related to the passport. Totp is only available for passports with a password. Basically totp is protecting passport, not otp over email or sms.`,
	},
	&cli.StringFlag{
		Name:     "totp-code",
		Required: true,
		Usage:    `The totp code generated by authenticator such as google or microsft apps.`,
	},
}
...
func CastConfirmClassicPassportTotpFromCli(c *cli.Context) *ConfirmClassicPassportTotpActionReqDto {
	...
	if c.IsSet("totp-code") {
		value := c.String("totp-code")
		template.TotpCode = &value
	}
	return template
}
...
var ClassicSigninCommonCliFlagsOptional = []cli.Flag{
	...
	&cli.StringFlag{
		Name:     "totp-code",
		Required: false,
		Usage:    `Accepts login with totp code. If enabled, first login would return a success response with next[enter-totp] value and ui can understand that user needs to be navigated into the screen other screen.`,
	},
	&cli.StringFlag{
		Name:     "session-secret",
		Required: false,
		Usage:    `Session secret when logging in to the application requires more steps to complete.`,
	},
}
...
var ClassicSignupCommonCliFlagsOptional = []cli.Flag{
	...
	&cli.StringFlag{
		Name:     "session-secret",
		Required: false,
		Usage:    `Required when the account creation requires recaptcha, or otp approval first. If such requirements are there, you first need to follow the otp apis, get the session secret and pass it here to complete the setup.`,
	},
	&cli.StringFlag{
		Name:     "type",
		Required: true,

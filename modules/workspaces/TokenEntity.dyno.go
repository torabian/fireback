  ...
  func TokenActionUpdateFn(query QueryDSL, fields *TokenEntity) (*TokenEntity, *IError) {
    ...
    if query.Tx == nil {
      ...
      vf := dbref.Transaction(func(tx *gorm.DB) error {
        dbref = tx
        _, err := TokenUpdateExec(dbref, query, fields)
        if err == nil {
          return nil
        } else {
          return err
...
var TokenCommonCliFlagsOptional = []cli.Flag{
  ...
  var TokenCreateCmd cli.Command = cli.Command{
    Name:    "create",
    Aliases: []string{"c"},
...
var TokenImportExportCommands = []cli.Command{
	...
	},
	{
		...
		Action: func(c *cli.Context) error {
			...
			TokenActionSeederInit(f, c.String("file"), c.String("format"))
			return nil
		},
	},
	{
    ...
    var TokenCliCommands []cli.Command = []cli.Command{
      GetCommonQuery(TokenActionQuery),

...
type GeoStateEntity struct {
	...
	Translations     []*GeoStateEntityPolyglot `json:"translations,omitempty" yaml:"translations,omitempty" gorm:"foreignKey:LinkerId;references:UniqueId;constraint:OnDelete:CASCADE"`
	Children         []*GeoStateEntity         `csv:"-" gorm:"-" sql:"-" json:"children,omitempty" yaml:"children,omitempty"`
...
func GeoStateEntityStream(q workspaces.QueryDSL) (chan []*GeoStateEntity, *workspaces.QueryResultMeta, error) {
	...
	q.ItemsPerPage = 50
	q.StartIndex = 0
	_, qrm, err := GeoStateActionQuery(q)
	if err != nil {
		return nil, nil, err
	}
	go func() {
...
type GeoStateEntityList struct {
	Items []*GeoStateEntity
}

func NewGeoStateEntityList(items []*GeoStateEntity) *GeoStateEntityList {
	return &GeoStateEntityList{
		Items: items,
	}
}
...
func (x *GeoStateEntity) Seeder() string {
	obj := GeoStateActionSeederInit()
	v, _ := json.MarshalIndent(obj, "", "  ")
	return string(v)
}
func GeoStateActionSeederInit() *GeoStateEntity {
	tildaRef := "~"
	_ = tildaRef
	entity := &GeoStateEntity{
		Name: &tildaRef,
...
var GeoStateAskCmd cli.Command = cli.Command{
	Name:  "nlp",
...
func GeoStateUpdateExec(dbref *gorm.DB, query workspaces.QueryDSL, fields *GeoStateEntity) (*GeoStateEntity, *workspaces.IError) {
	...
	cond2 := &GeoStateEntity{UniqueId: uniqueId}
	q := dbref.
		Where(cond2).
		FirstOrCreate(&item)
...
func GeoStatesActionQueryString(keyword string, page int) ([]string, *workspaces.QueryResultMeta, error) {
	...
	m := func(item *GeoStateEntity) string {
		label := item.UniqueId
		// if item.Name != nil {
		// 	label += " >>> " + *item.Name
		// }
...
func GeoStateCliFn() cli.Command {
	commands := append(GeoStateImportExportCommands, GeoStateCliCommands...)
	return cli.Command{
		Name:        "state",
...
var GeoStateEntityBundle = workspaces.EntityBundle{
	...
	//	GeoStateCliFn(),
	//},
	Actions:      GetGeoStateModule3Actions(),
	MockProvider: GeoStateImportMocks,

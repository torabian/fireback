...
type WalletPaymentAttemptOptionalDto struct {
	...
	Purpose emigo.Nullable[string] `json:"purpose" validate:"required,oneof=topup purchase withdrawal" yaml:"purpose"`
	...
	Status emigo.Nullable[string] `json:"status" validate:"required,oneof=pending requires_action succeeded failed cancelled expired" yaml:"status"`
...
func (x *WalletPaymentAttemptOptionalDto) Json() string {
	if x != nil {
...
func GetWalletPaymentAttemptOptionalDtoCliFlags(prefix string) []emigo.CliFlag {
...
func CastWalletPaymentAttemptOptionalDtoFromCli(c emigo.CliCastable) WalletPaymentAttemptOptionalDto {
	data := WalletPaymentAttemptOptionalDto{}
	...
	if c.IsSet("purpose") {
		emigo.ParseNullable(c.String("purpose"), &data.Purpose)
	}
	if c.IsSet("amount") {

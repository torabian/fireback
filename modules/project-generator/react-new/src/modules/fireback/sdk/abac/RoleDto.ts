...
export class RoleDto {
  ...
  applyFromObject(data = {}) {
    const d = data as Partial<RoleDto>;
  ...
  /**
   * Creates an instance of RoleDto, and possibleDtoObject
   * needs to satisfy the type requirement fully, otherwise typescript compile would
   * be complaining.
   **/
  static from(possibleDtoObject: RoleDtoType) {
    return new RoleDto(possibleDtoObject);
  }
  /**
   * Creates an instance of RoleDto, and partialDtoObject
   * needs to satisfy the type, but partially, and rest of the content would
   * be constructed according to data types and nullability.
   **/
  static with(partialDtoObject: PartialDeep<RoleDtoType>) {
    return new RoleDto(partialDtoObject);
  }
  copyWith(partial: PartialDeep<RoleDtoType>): InstanceType<typeof RoleDto> {
    return new RoleDto({ ...this.toJSON(), ...partial });
  }
  clone(): InstanceType<typeof RoleDto> {
    return new RoleDto(this.toJSON());
  }
}
export abstract class RoleDtoFactory {
  abstract create(data: unknown): RoleDto;
}
/**
 * The base type definition for roleDto
 **/
export type RoleDtoType = {
  /**
   *
   * @type {string}
   **/

...
export class WorkspaceConfigDto {
   ...
   * @type {string}
   **/
  set recaptcha2ServerKey(value: string) {
    this.#recaptcha2ServerKey = String(value);
  }
  setRecaptcha2ServerKey(value: string) {
  ...
  applyFromObject(data = {}) {
    const d = data as Partial<WorkspaceConfigDto>;
  ...
  /**
   * Creates an instance of WorkspaceConfigDto, and possibleDtoObject
   * needs to satisfy the type requirement fully, otherwise typescript compile would
   * be complaining.
   **/
  static from(possibleDtoObject: WorkspaceConfigDtoType) {
    return new WorkspaceConfigDto(possibleDtoObject);
  }
  /**
   * Creates an instance of WorkspaceConfigDto, and partialDtoObject
   * needs to satisfy the type, but partially, and rest of the content would
   * be constructed according to data types and nullability.
   **/
  static with(partialDtoObject: PartialDeep<WorkspaceConfigDtoType>) {
    return new WorkspaceConfigDto(partialDtoObject);
  }
  copyWith(
    partial: PartialDeep<WorkspaceConfigDtoType>,
  ): InstanceType<typeof WorkspaceConfigDto> {
    return new WorkspaceConfigDto({ ...this.toJSON(), ...partial });
  }
  clone(): InstanceType<typeof WorkspaceConfigDto> {
    return new WorkspaceConfigDto(this.toJSON());
  }
}
export abstract class WorkspaceConfigDtoFactory {
  abstract create(data: unknown): WorkspaceConfigDto;
}
/**
 * The base type definition for workspaceConfigDto
 **/
export type WorkspaceConfigDtoType = {
  /**
   *
   * @type {string}
   **/
...
export namespace WorkspaceConfigDtoType {}

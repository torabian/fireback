...
/**
 * Manages releases and release artifacts on Fireback.
 * @namespace FirebackReleases
 */
class Releases {
  /**
  ...
  /**
   * Registers a new release with fireback.
   *
   ...
   * @memberof FirebackReleases
   */
  async new(release, options) {
    const args = ["releases", "new", release].concat(
      helper.getProjectFlagsFromOptions(options)
    );
   ...
   * @param {string} options.repo The full repo name as defined in Fireback
   ...
   * this will default to the last commit of the previous release in Fireback. If there
  ...
  async setCommits(release, options) {
    ...
    return this.execute(
      ["releases", "set-commits", release].concat(commitFlags)
    );
  }
   ...
   *   // required options:
   *   env: 'production',          // environment for this release. Values that make sense here would be 
'production' or 'staging'
   *
   *   // optional options:
   *   started: 42,                // unix timestamp when the deployment started
   *   finished: 1337,             // unix timestamp when the deployment finished
   *   time: 1295,                 // deployment duration in seconds. This can be specified alternatively to 
`started` and `finished`
  ...
  async newDeploy(release, options) {
    ...
    return this.execute(
      helper.prepareCommand(args, DEPLOYS_SCHEMA, options),
      null
    );
  }
